...
def projection_simplex(V, z=1, axis=None):
    ...
    if axis == 1:
        ...
        U = mnp.sort(V, axis=1)[:, ::-1]
        z = mnp.ones(len(V)) * z
        cssv = mnp.cumsum(U, axis=1) - z[:, mnp.newaxis]
        ind = mnp.arange(n_features) + 1
        cond = U - cssv / ind > 0
        rho = mnp.count_nonzero(cond, axis=1)
        theta = cssv[mnp.arange(len(V)), rho - 1] / rho
        return mnp.maximum(V - theta[:, mnp.newaxis], 0)
...
class SquaredT(object):
    ...
    def delta_Omega(self, X):
        ...
        G = max_X / self.gamma
        return val, G
    def delta_Omega_Tensor(self,X):
        max_X = mnp.maximum(X, 0)
        val = mnp.sum(max_X ** 2, axis=0) / (2 * self.gamma)
        G = max_X / self.gamma
        return val, G
    def max_Omega(self, X, b):
        G = projection_simplex(X / (b * self.gamma), axis=0)
        val = mnp.sum(X * G, axis=0)
        val -= 0.5 * self.gamma * b * mnp.sum(G * G, axis=0)
        return val, G

    def Omega(self, T):
        return 0.5 * self.gamma * mnp.sum(T ** 2)
...
def solve_dual(u_c,u_d,C,regul, method="L-BFGS-B", tol=1e-3, max_iter=500):
        ...
        u_c = u_c.astype('float64').asnumpy()
        u_d = u_d.astype('float64').asnumpy()
        C = C.astype('float64').asnumpy()
        ...
        alpha = Tensor(res.x[:len(u_c)],dtype.float64)

        beta = Tensor(res.x[len(u_c):],dtype.float64)
...
def get_plan_from_dual(alpha, beta, C,regul):
        ...
        return regul.delta_Omega_Tensor(X)[1]
