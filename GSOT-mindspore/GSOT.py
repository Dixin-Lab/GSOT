...
def get_edges_from_plan(trans_plan:Tensor, max_num_edges):
    '''
    ...
    trans_plan_array = trans_plan.asnumpy()
    chain =np.sum(trans_plan_array, axis=2)
    ind = np.unravel_index(np.argsort(chain, axis=None)[::-1], chain.shape)
    #ind = mnp.unravel_index(ops.argsort(chain)[::-1], chain.shape)
    #ind = mnp.unravel_index(mnp.argsort(chain, axis=None)[::-1], chain.shape)
    edges = [[ind[0][k], ind[1][k] + num_src] for k in range(max_num_edges)]
    return Tensor(edges)
    #return mnp.array(edges)

def get_transport_plan(price: Tensor,
                       supply:  Tensor,
                       demand:  Tensor,
                       balance: bool=False,
                       alpha:   float=1,
                       rho:     float=1,
                       iterations:int = 50) -> Tuple[Tensor, Tensor, float]:
    """
...
def evaluate_net_profit(sorted_edges: Tensor,
                        price: Tensor,
                        supply: Tensor,
                        demand: Tensor,
                        integer: bool = False) -> Tensor:
    """
    ...
    num_pairs = supply.shape[1]
    values = mnp.zeros((max_num_edges, num_pairs),dtype.float64)
    ...
    net_profit=mnp.mean(values, axis=1).max()
    return values,net_profit



def plot_network(path_name: str, num_src: int, num_dst: int, edge: Tensor ) :
    """
